...
import FormattedMessageWrapper from '../../../common/FormattedMessageWrapper/FormattedMessageWrapper';
...
class AdminContents extends React.Component {
    ...
    render() {
        ...
        let headings = [
            <FormattedMessageWrapper message={intlStore.getMessage(intlData, 'typeHeading')}
                              locales={intlStore.getCurrentLocale()} />,
            <FormattedMessageWrapper message={intlStore.getMessage(intlData, 'nameHeading')}
                              locales={intlStore.getCurrentLocale()} />,
            <FormattedMessageWrapper message={intlStore.getMessage(intlData, 'tagsHeading')}
                              locales={intlStore.getCurrentLocale()} />,
            <FormattedMessageWrapper message={intlStore.getMessage(intlData, 'enabledHeading')}
                              locales={intlStore.getCurrentLocale()} />
        ];
            ...
            return {
                data: [
                    <Text size="medium">
                        {content.type ?
                            <Label>
                                <FormattedMessageWrapper message={intlStore.getMessage(intlData, content.type)}
                                                  locales={intlStore.getCurrentLocale()} />
                            </Label>
                            :
                            <Label type="error">
                                <FormattedMessageWrapper message={intlStore.getMessage(intlData, 'noType')}
                                                  locales={intlStore.getCurrentLocale()} />
                            </Label>
                        }
                    </Text>,
                    <span className="admin-contents__link">
                        <Link to="adm-content-edit" params={Object.assign({contentId: content.id}, routeParams)}>
                            <FormattedMessageWrapper message={intlStore.getMessage(content.name)}
                                              locales={intlStore.getCurrentLocale()} />
                        </Link>
                    </span>,
                        ...
                        {content.tags.map(function (tag, idx) {
                            return (
                                <div key={idx} className="admin-contents__tag">
                                    <Label>
                                        <FormattedMessageWrapper message={intlStore.getMessage(intlData, tag)}
                                                          locales={intlStore.getCurrentLocale()} />
                                    </Label>
                                </div>
                            );
