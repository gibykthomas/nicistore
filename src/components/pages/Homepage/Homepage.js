...
import FormattedMessageWrapper from '../../../components/common/FormattedMessageWrapper/FormattedMessageWrapper';
...
import testStore from '../../../stores/Collections/testStore';
...
class Homepage extends React.Component {
    ...
    state = {
        ...
        // checking: this.context.getStore(testStore).getCollections(['collection', 'homepage'])
    };
    ...
    render() {
        ...
        for (let i=0; i<4; i++) {
            if (this.state.collections[i]) {
                ...
                featuredCollections[i] = {
                    ...
                    link: {
                        to: `/${routeParams.locale}/collections/${collection.id}/${slugify(intlStore.getMessage(collection.name))}/?`,
                    }
                };
        ...
        let featuredProductsTitle = function() {
            return <FormattedMessageWrapper 
 ...
 * Flux
 */
Homepage = connectToStores(Homepage, [testStore, ContentsListStore, CollectionsStore, ProductsHomepageStore], (context, props) => {
    return {
        ...
        _checking: context.getStore(testStore).getProducts(),
    };
});
